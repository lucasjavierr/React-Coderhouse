...
const ItemListContainer = ({ greeting }) => {
  ...
  if (!categoryId && products.leght === 0) {
    return (
      <h1 style={{ textAlign: "center", fontSize: "3rem" }}>404 NOT FOUND</h1>
    );
  }
  ...
  return (
    <section className={s.container}>
      ...
      <div className={s.buttonsContainer}>
        <div>
          <button onClick={() => setDisplayList(true)}>
            <i className="bi bi-list-task"></i>
          </button>
          <button onClick={() => setDisplayList(false)}>
      ...
      {displayList ? (
        <ItemList products={products} displayList />
      ) : (
        <ItemGrid products={products} displayList={displayList} />
      )}
    </section>
  );
};
